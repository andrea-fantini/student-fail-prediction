...
When comparing the preformance of the two models two feature sets with the baseline of using all the features, we observe that the variables selected with Lasso yelds a higher score and lower variance, thus we will use the following 4 features: 
...
Given the nature of the data and the type of classification problem, we focus the modeling efforts on tree based models. We compared the Scikit-Learn implementation of the Random Forest for the Bagging ensambling technique, and the XGBoost for the Gradient Descent Boosting approach.

In both cases we use Bayesian Optimization with crossavlidation to find the best performing hyper-parameters while imiting overfitting. To compare the performance of the two models we calculate the Area Under the Receiving Operating Characteristic curve. 

![AUC-ROC](fig/Comparison%20ROC%20-%20precision.png)

As we can see the Random Forest capures a slightly larger are under the ROC curve.
...
Based on these findings we conclude that the optimal model for this prediciton problem is a Random Forest Classifier with the following parameters:
```
...
```
And using the following 4 features to infor the prediciton:
```
- hours_with_views
- avg_views_per_hour
- tot_participations
